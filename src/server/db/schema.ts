...
export const goalPriorityEnum = pgEnum('goal_priority', ['priority_1', 'priority_2', 'priority_3', 'priority_4'])
...
export const goalTypeEnum = pgEnum('goal_type', ['daily', 'weekly', 'monthly', 'yearly'])
...
export const goals = createTable(
  'goal',
  {
    ...
    priority: goalPriorityEnum('priority').default(goalPriorityEnum.enumValues[0]).notNull(),
    impact: goalImpactEnum('impact').default('medium').notNull(),
    category: goalCategoryEnum('category').default('health').notNull(),
    type: goalTypeEnum('type').default(goalTypeEnum.enumValues[0]).notNull(),
  ...
  (goal) => ({
    userTitleIndex: uniqueIndex('goal_title_user_id_unique_idx').on(goal.userId, goal.title),
...
export const habits = createTable(
  ...
  (habit) => ({
    userTitleIndex: uniqueIndex('habit_title_user_id_unique_idx').on(habit.userId, habit.title),
    userIdIdx: index('habit_user_id_idx').on(habit.userId),
  }),
)

export const habitTrackerEnum = pgEnum('habit_tracker', ['daily', 'weekly', 'monthly'])

export const habitTrackerStatusEnum = pgEnum('habit_tracker_status', ['pending', 'completed'])
...
export const habitTracker = createTable(
  ...
  (habitTracker) => ({
    habitIdUserIdIdx: index('habit_tracker_habit_id_user_id_idx').on(habitTracker.habitId, habitTracker.userId),
...
export const journal = createTable(
  ...
  (journal) => ({
    userTitleIndex: uniqueIndex('journal_title_user_id_unique_idx').on(journal.userId, journal.title),
...
export const journalGoals = createTable(
  ...
  (journalGoal) => ({
    ...
    journalGoalIdx: uniqueIndex('journal_goal_idx').on(journalGoal.journalId, journalGoal.goalId),
  }),
)
...
export const journalHabits = createTable(
  ...
  (journalHabits) => ({
    journalHabitIdx: uniqueIndex('journal_habit_idx').on(journalHabits.journalId, journalHabits.habitId),
