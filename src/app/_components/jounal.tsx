...
export const Journals: React.FC = () => {
  ...
  const { mutate: deleteJournal, isPending: isDeletingJournal, variables: deleteJournalVariables } = api.journals.delete.useMutation({
    onSuccess: () => {
      refetchJournals()
    },
  })
    ...
    <div className="space-y-2">
        ...
        {journals?.map((journal) => (
          ...
          <div  className="p-3 pl-1 mb-1 rounded-lg border relative hover:shadow-md">
            ...
            <div className="absolute bottom-0.5 right-0">
              <Button variant="ghost" size={'sm'} onClick={() => handleEditJournal(journal.id)}><Edit3 className="h-4 w-4" /></Button>
              <Button variant="ghost" size={'sm'} className=' hover:bg-red-900 active:bg-red-900' onClick={() => deleteJournal({ id: journal.id })} disabled={isDeletingJournal && deleteJournalVariables?.id === journal.id} loading={isDeletingJournal && deleteJournalVariables?.id === journal.id} hideContentWhenLoading><Trash2 className="h-4 w-4" /></Button>
            </div>
          </div>
          <div className="text-end text-sm text-muted-foreground pr-2 pt-1">
              {journal.createdAt.toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric' })}
            </div>
          </div>
        ))}
      </ScrollArea>
      <Dialog open={isCreateJournalOpen} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[800px]">
          ...
          <DialogFooter>
            <Button onClick={() => setIsCreateJournalOpen(false)} size={'sm'} variant="outline">Cancel</Button>
