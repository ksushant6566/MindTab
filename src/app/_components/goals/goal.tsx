...
import { Edit3, Flag, Trash2, Zap } from 'lucide-react'
...
export const Goal: React.FC<GoalProps> = ({ goal, onEdit, onDelete, onToggleStatus, isDeleting, deleteVariables }) => {
  return (
    <div className="group relative grid grid-cols-[auto,1fr] gap-3 items-start">
      ...
      <div className="flex flex-col gap-1 min-w-0">
        <Label className={`text-sm font-medium ${goal.status === 'completed' ? 'line-through' : ''}`}>
          {goal.title}
        </Label>
        {goal.description && (
          <p
        ...
        <div className="mt-1 flex items-center gap-2 flex-wrap">
          <span className="flex items-center gap-1 rounded-md bg-secondary px-1 py-0.5 text-xs capitalize text-muted-foreground">
            <Flag
              ...
              fill={priorityColors[goal.priority as keyof typeof priorityColors]}
            />
      ...
      <div className="absolute right-0 bottom-0 flex z-10 -translate-y-6 gap-0 opacity-0 transition-all group-hover:translate-y-1 group-hover:opacity-100">
        <Button size="sm" variant="ghost" className="h-8 w-8 p-0.5" onClick={() => onEdit(goal.id)}>
